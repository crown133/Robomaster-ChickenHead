...
#define MPU6500_Write_Reg_Num 15

static const uint8_t write_mpu6500_reg_data_error[MPU6500_Write_Reg_Num][3] =
    {
		{MPU_PWR_MGMT_1,
         (((~MPU_DEVICE_RESET) & (~MPU_SLEEP) & (MPU_CYCLE) & (~MPU_GYRO_STANDBY) & (~MPU_TEMP_DISABLE)) | (MPU_CLKSEL_BEST_PLL)),
         PWR_MGMT_1_ERROR}, //使能温度 使用gyroX振荡器时钟

        {MPU_PWR_MGMT_2,
         (((~MPU_DISABLE_XA) & (~MPU_DISABLE_YA) & (~MPU_DISABLE_ZA) & (~MPU_DISABLE_XG) & (~MPU_DISABLE_YG) & (~MPU_DISABLE_ZG)) & (MPU_LP_WAKE_1_25_HZ)),
         PWR_MGMT_2_ERROR},//使能六轴 加速度计唤醒频率1.25Hz

        {MPU_SMPLRT_DIV,
         MPU_SMPLRT_DIV_1,
         SMPLRT_DIV_ERROR},//采样频率=陀螺仪输出频率/（1+SIMPLRT_DIV）
//                         //加速度计输出频率为1KHz，故若采样率大于1K，同一加速度计可能被多次采样

        {MPU_CONFIG,
         ((~MPU_FIFO_MODE_OFF_REPLACE_OLD_DATA) & (MPU_EXT_SYNC_DISABLE | MPU_DLPF_CFG_2_SET)),
         CONFIG_ERROR}, //数字低通滤波器为2
        ...
        {MPU_ACCEL_CONFIG_2,
         ...
         ACCEL_CONFIG_2_ERROR}, //Page15  低通滤波带宽460Hz Rate1kHz
		...
		{MPU_FIFO_EN,
		 (((MPU_FIFO_TEMP_OUT_ENABLE) | (MPU_FIFO_GYRO_X_OUT_ENABLE) | (MPU_FIFO_GYRO_Y_OUT_ENABLE) |\
		  (MPU_FIFO_GYRO_Z_OUT_ENABLE) | (MPU_FIFO_ACCEL_OUT_ENABLE)) & ((~MPU_FIFO_SLV_2_OUT_ENABLE) &\
		  (~MPU_FIFO_SLV_1_OUT_ENABLE) & (~MPU_FIFO_SLV_0_OUT_ENABLE))),
		 FIFO_EN_ERROR},  //FIFO缓冲区存入数据
		
        {MPU_I2C_MST_CTRL,
         (((~MPU_MULT_MST_EN) & (~MPU_SLV_3_FIFO_EN) & (~MPU_I2C_MST_P_NSR)) & (MPU_WAIT_FOR_ES_EN | MPU_I2C_MST_CLK_400_KHZ)),
         I2C_MST_CTRL_ERROR}, //if use external sensor, set MPU_WAIT_FOR_ES

        {MPU_INTBP_CFG,
         ((~MPU_INTBP_ACTL) & (~MPU_INTBP_OPEN) & (~MPU_LATCH_INT_EN) & (~MPU_INT_ANYRD_2CLEAR) & (~MPU_ACTL_FSYNC) & (~MPU_FSYNC_INT_MODE_EN)) & (MPU_BYPASS_EN),
         INTBP_CFG_ERROR}, //configure the interrupt pin

        {MPU_INT_ENABLE,
         (((MPU_FIFO_OVERFLOW_EN) & (~MPU_FSYNC_INT_EN)) & (~MPU_RAW_RDY_EN)),
         INT_ENABLE_ERROR}, //Enable FIFO_OV interrupt 

        {MPU_I2C_MST_DELAY_CTRL,
         MPU_I2C_SLV0_DLY_EN,
         I2C_MST_DELAY_CTRL_ERROR}, // slave 0 will only be accessed at a decreased rate
        ...
        {MPU_USER_CTRL,
         (((~MPU_DMP_EN) & (~MPU_DMP_RST) & (~MPU_FIFO_RST) & (~MPU_I2C_MST_RST) & (~MPU_SIG_COND_RST)) & (MPU_FIFO_MODE_EN | MPU_I2C_MST_EN | MPU_I2C_IF_DIS)),
         USER_CTRL_ERROR},  //FIFO Mode 使能
};

uint8_t mpu6500_init(void)
{
    uint8_t res = 0;
    uint8_t wait_time = 200;
    uint8_t sleepTime = 50;

    uint8_t writeNum = 0;

    //mpu6500 Nss gpio init
    mpu6500_GPIO_init();
