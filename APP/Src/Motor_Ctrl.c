...
//	motorYaw.veloCtrl.refVel = motorYaw.posCtrl.output;
	...
	/************** Jscope 查看参数用 *****************/
//	posRaw = (int)motorYaw.posCtrl.rawPos*1000;
//	posOut = (int)motorYaw.posCtrl.output*1000;
//	velRef = (int)td1.v1*1000;
//	velOut = (int)motorYaw.veloCtrl.output*1000;
	/**********************************/
...
void motorYawCalcuPos(void)
{
	...
	motorYaw.posCtrl.err = motorYaw.posCtrl.refPos + motorYaw.posCtrl.motorBias - motorYaw.posCtrl.relaPos;
	{	
		// 计算积分值，注意末尾积分限幅 */
...
void motorPitchCalcuPos(void)
{
	...
	{	
		...
		motorPitch.posCtrl.output = motorPitch.posCtrl.kp * motorPitch.posCtrl.err * abs(motorPitch.posCtrl.err) + motorPitch.posCtrl.ki * motorPitch.posCtrl.integ + motorPitch.posCtrl.kd * diff;
		
		if(motorPitch.posCtrl.output > 100)
		{
			motorPitch.posCtrl.output += 100;
		}
